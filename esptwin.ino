...
void setup() 
{
  ...
  if (digitalRead(10)==LOW) { 
    ...
    http.end();
    // http.begin("http://php.esptwin.com/insertsql.php?channel=100000000&esp=361413&data=Test3");
    // httpCode = http.GET();
    // if (httpCode > 0) {
    //   Serial.printf("[HTTP] GET ... code: %d\n", httpCode);
    //   if(httpCode == HTTP_CODE_OK) {
    ...
    // }
    // http.end();
    http.begin("http://php.esptwin.com/readdata.php?channel=100000000");
    httpCode = http.GET();
    if (httpCode > 0) {
      String payload;
      Serial.printf("[HTTP] GET ... code: %d\n", httpCode);
      ...
      data = payload.substring(payload.indexOf('¥')+1,payload.lastIndexOf('¥')-1);
      Serial.println(data);
      Twin_S.println(data);
    }        
  } else {
